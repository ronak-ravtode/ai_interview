                ...
                <div className='flex items-center gap-6 relative'>
                    <div className='relative'>
                        <button onClick={() => {
                            ...
                            setShowUserPopup(false)
                        }} className='flex items-center gap-2 py-2 px-4 rounded-full bg-gray-100 hover:text-gray-600 text-md transition'>
                            <BsCoin size={20} />
                            {userData?.user?.credits || 0}
                        </button>
                        {showCreditsPopup && (
                            <div className='absolute mt-3 right-[-50px] w-64 bg-white border border-gray-200 rounded shadow-xl p-4 z-50'>
                    ...
                    <div className='relative'>
                        <button onClick={() => { 
                            ...
                            setShowUserPopup(!showUserPopup); setShowCreditsPopup(false) }} className='w-9 h-9 bg-black text-white rounded-full flex items-center justify-center font-semibold'>
                            {userData?.user ? userData.user.name.slice(0, 1).toUpperCase()
